...
import {
  ...
  useGetPlants,
  useUpdatePlant,
} from "../../domains/plants/plants.hooks";
import {
  useCreateZombie,
  useDeleteZombie,
  useGetZombies,
  useUpdateZombie,
} from "../../domains/zombies/zombies.hooks";
import {
  useCreateMap,
  useDeleteMap,
  useGetMaps,
  useUpdateMap,
} from "../../domains/maps/maps.hooks";
...
export default function EndpointsPage(): JSX.Element {
    ...
    const mockEndpoints: Endpoint[] = [
      {
        ...
        path: "/plantes",
        method: "GET",
        description: "Récupérer toutes les plantes",
        status: "pending",
      },
      {
        id: "2",
        path: "/zombies",
        method: "GET",
        description: "Récupérer tous les zombies",
        status: "pending",
      },
      {
        id: "3",
        path: "/maps",
        method: "GET",
        description: "Récupérer toutes les maps",
        status: "pending",
      },
      {
        id: "4",
        path: "/plantes",
        method: "POST",
        description: "Créer une nouvelle plante",
        status: "pending",
      },
      {
        id: "5",
        path: "/zombies",
        method: "POST",
        description: "Créer un nouveau zombie",
        status: "pending",
      },
      {
        id: "6",
        path: "/maps",
        method: "POST",
        description: "Créer une nouvelle map",
        status: "pending",
      },
      {
        id: "7",
      ...
      },
      {
        ...
        path: "/zombies/1",
        method: "PUT",
        description: "Modifier un zombie",
        status: "pending",
      },
      {
        id: "9",
      ...
      },
      {
        ...
        path: "/zombies/1",
        method: "DELETE",
        description: "Supprimer un zombie",
        status: "pending",
      },
      {
        id: "12",
        path: "/maps/1",
        method: "DELETE",
        description: "Supprimer une map",
        status: "pending",
      },
      {
        id: "13",
        path: "/plantes/validation",
        method: "GET",
        description: "Valider le format des plantes retournées",
        status: "pending",
      },
      {
        id: "14",
        path: "/zombies/validation",
        method: "GET",
        description: "Valider le format des zombies retournés",
        status: "pending",
      },
      {
        id: "15",
        path: "/maps/validation",
        method: "GET",
        description: "Valider le format des maps retournées",
        status: "pending",
      },
    ];
  ...
  const validatePlantStructure = (data: unknown): ValidationResult => {
    ...
    if (
      ...
      return {
        valid: false,
        message: `Données invalides: attendu un objet ou un tableau, reçu ${typeof parsedData}`,
      };
    }
    const items: unknown[] = Array.isArray(parsedData)
      ? parsedData
      : [parsedData];
    if (items.length === 0)
      return { valid: true, message: "Aucune plante à valider" };
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (const field of requiredFields) {
        if (!(field in record)) {
          errors.push(`Plante #${i}: champ '${field}' manquant`);
          continue;
        }
        ...
        if (
          ...
          if (typeof value !== "number")
            errors.push(
              `Plante #${i}: champ '${field}' devrait être un nombre, reçu ${typeof value}`
            );
        } else if (
          field === "attaque_par_seconde" ||
          field === "cout" ||
          field === "soleil_par_seconde"
        ) {
          if (
            typeof value !== "number" &&
            ...
            errors.push(
              `Plante #${i}: champ '${field}' devrait être un nombre ou une chaîne convertible en nombre, reçu ${typeof value}`
            );
        } else if (
          field === "nom" ||
          field === "effet" ||
          ...
          if (typeof value !== "string")
            errors.push(
              `Plante #${i}: champ '${field}' devrait être une chaîne, reçu ${typeof value}`
            );
        }
      }
    }
    return {
      ...
      message:
        errors.length === 0
          ? "Toutes les plantes sont valides"
  ...
  const validateZombieStructure = (data: unknown): ValidationResult => {
    ...
    const items: unknown[] = Array.isArray(parsedData)
      ...
      : [parsedData];
    if (items.length === 0)
      return { valid: true, message: "Aucun zombie à valider" };
    const requiredFields = [
      "id_zombie",
      "nom",
      "point_de_vie",
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (const field of requiredFields) {
        if (!(field in record)) {
          errors.push(`Zombie #${i}: champ '${field}' manquant`);
          continue;
        }
        const value = record[field];
        if (
          ...
          if (typeof value !== "number")
            errors.push(
              `Zombie #${i}: champ '${field}' devrait être un nombre, reçu ${typeof value}`
            );
        } else if (
          field === "attaque_par_seconde" ||
          field === "vitesse_de_deplacement"
        ) {
          if (
            typeof value !== "number" &&
            (typeof value !== "string" || isNaN(Number(value)))
            ...
            errors.push(
              `Zombie #${i}: champ '${field}' devrait être un nombre ou une chaîne convertible en nombre, reçu ${typeof value}`
            );
        } else if (field === "nom" || field === "chemin_image") {
          if (typeof value !== "string")
            errors.push(
              `Zombie #${i}: champ '${field}' devrait être une chaîne, reçu ${typeof value}`
            );
        }
      }
      if (
        "id_map" in record &&
        record["id_map"] !== null &&
        record["id_map"] !== undefined
      ) {
        const idMap = record["id_map"];
        if (typeof idMap !== "number")
          errors.push(
            `Zombie #${i}: champ 'id_map' devrait être un nombre, reçu ${typeof idMap}`
          );
      }
    }
    return {
  ...
  const validateMapStructure = (data: unknown): ValidationResult => {
    ...
    const items: unknown[] = Array.isArray(parsedData)
      ...
      : [parsedData];
    if (items.length === 0)
      return { valid: true, message: "Aucune map à valider" };
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (const field of requiredFields) {
        if (!(field in record)) {
          errors.push(`Map #${i}: champ '${field}' manquant`);
          continue;
        }
        const value = record[field];
        if (field === "id_map" || field === "ligne" || field === "colonne") {
          if (typeof value !== "number")
            errors.push(
              `Map #${i}: champ '${field}' devrait être un nombre, reçu ${typeof value}`
            );
        } else if (field === "chemin_image") {
          if (typeof value !== "string")
            errors.push(
              `Map #${i}: champ '${field}' devrait être une chaîne, reçu ${typeof value}`
            );
        }
      }
    }
    ...
    try {
      ...
      switch (endpoint.method) {
        ...
        case "POST":
          ...
          if (endpoint.path === "/zombies") {
            response = await createZombie({
              nom: "Nouveau zombie",
              point_de_vie: 150,
              attaque_par_seconde: 1.0,
              degat_attaque: 15,
              vitesse_de_deplacement: 1.5,
              chemin_image: "/images/zombies/default.png",
              id_map: 1,
            });
            validationResult = {
              valid: true,
              message: "Zombie créé avec succès",
            };
          }
          if (endpoint.path === "/maps") {
        ...
        case "PUT":
          ...
          if (endpoint.path === "/zombies/1") {
            ...
            validationResult = {
              valid: true,
              message: "Zombie modifié avec succès",
            };
          }
          if (endpoint.path === "/maps/1") {
            response = await updateMap({
              id_map: 1,
              data: { ligne: 6, colonne: 10 },
            });
            validationResult = {
              valid: true,
              message: "Map modifiée avec succès",
            };
          }
          break;
        case "DELETE":
          ...
          if (endpoint.path === "/zombies/1") {
            ...
            validationResult = {
              valid: true,
              message: "Zombie supprimé avec succès",
            };
          }
          if (endpoint.path === "/maps/1") {
            await deleteMap(1);
            validationResult = {
              valid: true,
              message: "Map supprimée avec succès",
            };
          }
          break;
        default:
      ...
      setEndpoints((prev) =>
        prev.map((e) =>
          e.id === endpoint.id
            ? { ...e, status: "success", validationResult }
            : e
        )
      );
    } catch (error) {
        ...
        prev.map((e) =>
          e.id === endpoint.id
            ? {
                ...
                validationResult: {
                  valid: false,
                  message: "Erreur lors de l'exécution de l'opération",
                },
              }
            : e
        )
